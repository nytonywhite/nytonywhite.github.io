(function(){
	...
	var talentLoader = {
		...
		infoShow:function(container,talentGameInfo){
			...
			loaderHeaderLeftMenuObj.style['display'] = 'none';
			...
			loaderHeaderLeftMenuObj.onclick = function(e){catalogMask.show();cancelBubble(e);}
			loaderHeaderLeftMenuObj.ontouchstart = function(e){catalogMask.show();cancelBubble(e);};
			...
			loaderAdContainer.innerHTML+='<style>#talentContainer #loaderMask{overflow-y:scroll;} #talentContainer #loaderMask.expand{overflow-y:scroll !important;} #talentContainer #loaderMask.expand #loaderInfo{height:auto !important;} #talentContainer #loaderAdBlock>ins{  border: 1px solid #ddd;position: absolute;top: 50%;margin-top: -140px;left: 50%;  margin-left: -168px;}</style>';
	...
	function init(){
		...
		var basePath = '../../gamelist/';
		var gameid = getQueryString('gameid')|| '500002';
		if(window.talentGameInfo) talentGameInfo.banner = basePath+gameid+'/banner_t.jpg';
		var mainContainer = document.getElementById('mainContainer');
		mainContainer.src = basePath+gameid+'/index.html';

		talentLoader.show(talentContainer);

		if(window.talentGameInfo){
			catalogMask.init(talentContainer);
		} 

		// console.log(talentGameInfo);
	...
	function initMediation(){
	    ...
	    window.backToGameList = function (){
	    	window.history.back()
	        //window.location.href = window.basePath+'index.html?s='+sign+'&appId='+appId+menuString+'&'+talentGA.jumpKey+'='+talentGA.jumpType.game;
	    }
	        ...
	        Util.mobvistaAjax({
	            ...
	            success: function (data) {
	                ...
	                for(var i=0;i<gamesIcon.length;i++){
	                    ...
	                    var targetUrl = 'index.html?gameid='+game.gameId;

	                    Util.domHelper(gamesIcon[i]).attr({
	                        'href':targetUrl
	                    }).css({
	                        'background-image':'url(../../gamelist/'+gameIconUrl+')'
	                    });

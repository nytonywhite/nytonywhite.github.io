...
var mediationHost = 'js/profile.json';
...
var Mediation = {
	...
	init: function(param) {
		...
		if(me.mediaWeightArray.length==0){
			...
			var callbackKey = '1435315441000028711936436593533';
			Util.mobvistaJSONPAjax.load({
				...
				success: function(result) {
					...
					if(result.msg=='success'){
						...
						if (ga) ga('send', 'event', 'load', 'profile', 'load mediation success');
					}else if(result.status==-1){
						// me.defaultMedia(param,'mobvista');
						me.defaultMedia(param);
						if (ga) ga('send', 'event', 'load', 'profile', 'checked no mediation');
					}else if(result.status==-4){
						me.defaultMedia(param);
						if (ga) ga('send', 'event', 'load', 'profile', 'checked faile');
					}else{
						me.defaultMedia(param);
						if (ga) ga('send', 'event', 'load', 'profile', 'checked faile others');
					}
				...
				error:function(){
					me.defaultMedia(param);
					if(param.callback) param.callback();
					if (ga) ga('send', 'event', 'load', 'profile', 'connect faile');
				}
			});
		}else{
			me.getParams();

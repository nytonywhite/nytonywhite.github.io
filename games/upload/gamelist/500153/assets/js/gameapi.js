...
fg_api.prototype.adsModule = function() {
	...
	module.prototype.init = function() {
		...
		self.log("M.show_video", M.show_video);
		...
		switch (M.provider) {
			...
			case 'adx':
				M.provider = 'dfp';
			case 'dfp':
				faZepto.getScript("assets/js/ima3.js",
					sdkLoadedCallback,
					function() {
						sdkLoadedCallback();
	...
	module.prototype.showClick2PlayOverlay = function(callback, force) {
		...
		var friendly_name = JSON.parse(self.config.name);
	...
	module.prototype.showIMA = function(callback, force) {
		...
		M.adMinHeight = 250 + 65;
		M.adMaxHeight = Math.min(window.innerHeight - 65, 480);
		M.adHeight = Math.max(M.adMaxHeight, Math.min(M.adMinHeight, M.adMaxHeight)) + "px";
...
fg_api.prototype.orientationModule = function() {
	...
	orientationPrototype.init = function(){
		...
		if (typeof self.config.gameParams.orientation != 'undefined' &&
			...
			fgLandscapeImage.setAttribute("src", "RotateToLandscape.png");
			...
			fgPortraitImage.setAttribute("src", "RotateToPortrait.png");
			...
			self.handleClick(fgLandscapeOverlay, function() {
				self.fullscreen.start();
				self.orientation.lock();
			});
			self.handleClick(fgPortraitOverlay, function() {
				self.fullscreen.start();
				self.orientation.lock();
			});
...
fg_api.prototype.init = function() {
	...
	moreGamesImg.onerror = function() {
		if (self.config.game_i18n.current) {
			self.config.game_i18n.current.more_games_image = "600x253.png";
		}
		advance();
	};
    ...
    function advance() {
		...
		newStylesheet.setAttribute("href", "assets/css/play.css");
